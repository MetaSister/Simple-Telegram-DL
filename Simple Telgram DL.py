...
def load_or_create_config():
    ...
    if not os.path.exists(CONFIG_FILE):
        print("Telegram API details not found. Please enter the following details.")
        url = "https://my.telegram.org"
        print(f"You can get these details from: \033]8;;{url}\a{url}\033]8;;\a")
        try:
            ...
            if api_id and api_hash:
                ...
                try:
                    with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
                        config.write(f)
                    print(f"API details saved to '{CONFIG_FILE}'.")
                    return api_id, api_hash
                except IOError as e:
                    print(f"\nERROR: Could not write to config file: {e}. Please check file permissions.\n")
                    return None, None
            else:
                print("\nERROR: API ID and API HASH cannot be empty. Please try again.\n")
    else:
    ...
    try:
        async for message in client.iter_messages(target_entity, reverse=True, offset_date=start_date):
            ...
            if message.media:
                ...
                if should_download:
                    print(f"\n(Found media #{downloaded_count + 1} - Message Date: {message.date.strftime('%Y-%m-%d')})")
...
def reset_configuration():
    ...
    confirm = input(f"This will delete '{CONFIG_FILE}' and '{session_file_to_delete}'. Are you sure? (y/n): ").lower().strip()
    if confirm == 'y':
        try:
            ...
            print("\nConfiguration successfully reset. You can enter new information when you restart the program.")
        except OSError as e:
    ...
    while True:
        ...
        if main_choice == '1':
            ...
            if not api_id or not api_hash:
                print("\nCould not load API details. Please proceed from the main menu.")
                continue
            ...
            try:
                ...
                print("\nPlease check your API details, phone number, and password.")
                print(f"If the problem persists, try resetting the configuration using option [2] from the main menu.")
            finally:
